...
import tensor_core_mma_cmodel_debug as tensor_core_mma_cmodel
import random_case_generator as random_case_generator
...
class NumericalStrategy(TestCase):
    ...
    def mma_kernel_init(self):
        ...
        self.mod = SourceModule(
            cuda_code, options=["-arch=sm_120a", "--cubin"], keep=True
        )
    ...
    def run_test_case(self):
        ...
        return flag, output[0][0], cmodel_result

    def save_failed_case(self, h_a, h_b, h_c, output, cmodel_result):
        """保存失败的测试用例到 JSON 文件"""
        # 确保保存目录存在
        if not os.path.exists("fp8_failed_cases"):
            os.makedirs("fp8_failed_cases")
        ...
        file_path = f"fp8_failed_cases/failed_case_{time.time_ns()}.json"
    ...
    def load_and_run_failed_case(self, file_path):
        ...
        #     order="F",
        # )
        print(input_type[self.dtype_dict["B"]])
        h_b = np.zeros(
            (self.mma_shape[2], self.mma_shape[1]),
            dtype=input_type[self.dtype_dict["B"]],
            order="F",
    ...
    def test_cases_from_yaml(self):
        ...
        for k in range(test_case_num):
            ...
            result, gpu_output, cmodel_output = self.run_test_case()
            ...
            if result == False:
                ...
                results.append(
                    ...
                    + str(result)
                    + "\n"
                    + f"{gpu_output:032b}"[:22]
                    + "\n"
                    + f"{int(cmodel_output):032b}"[:22]
                )
            else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print(
            "使用方法: python general_cmodel_precision_check.py <type_A> <type_B> <type_C>"
        )
        sys.exit(1)
    dtype_A = sys.argv[1]
    dtype_B = sys.argv[2]
    ...
    # print(dtype_dict)
    failed_cases = []
    test_run_func(failed_cases, dtype_dict)
